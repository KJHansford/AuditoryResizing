...
\vspace{5mm}
*Keywords*: Multisensory Processing, Resizing Illusions, Proprioceptive Drift, Audition. 
...
for (s in 1:length(sublist)){
  ...
  for (trial in 1:length(triallist)){
    ...
    if (triallist[trial]>6){
      ...
      # checks if the rating was in the range of the ruler, flags if not
...
# these lines convert out of range ruler ratings to NA values
...
# Create group labels from earlier data (rather than hard coding)
Group <- as.character(groupassignments)
Group_OR <- as.character(groupassignments_OR)
...
allsettingsright_formatted <- melt(allsettingsrightdf, id.vars=c("Group", "PID"), measure.vars=c("RightDotBaseline", "RightDotV/VA", "RightDotVT/VTA" ), variable.name="condition", value.name="score")

# Sort by subject first, then by condition
allsettingsright_formatted <- allsettingsright_formatted[order(allsettingsright_formatted$PID,                        allsettingsright_formatted$condition),]

allsettingsright_formatted_OR <- melt(allsettingsrightdf_OR, id.vars=c("Group_OR", "PID_OR"), measure.vars=c("RightDotBaseline", "RightDotV/VA", "RightDotVT/VTA" ), variable.name="condition", value.name="score")
...
allsettingsleft_formatted <- melt(allsettingsleftdf, id.vars=c("Group", "PID"), measure.vars=c("LeftDotBaseline", "LeftDotV/VA", "LeftDotVT/VTA" ), variable.name="condition", value.name="score")
...
allsettingsleft_formatted_OR <- melt(allsettingsleftdf_OR, id.vars=c("Group_OR", "PID_OR"),measure.vars=c("LeftDotBaseline", "LeftDotV/VA", "LeftDotVT/VTA" ), variable.name="condition", value.name="score")
...
allsettingsruler_formatted <- melt(allsettingsrulerdf,id.vars=c("Group", "PID"), measure.vars=c("RulerBaseline", "RulerV/VA", "RulerVT/VTA" ),variable.name="condition",
value.name="score")
...
allsettingsruler_formatted_OR <- melt(allsettingsrulerdf_OR,id.vars=c("Group_OR", "PID_OR"), measure.vars=c("RulerBaseline", "RulerV/VA", "RulerVT/VTA" ), variable.name="condition", value.name="score")
...
normsettingsright_formatted_OR <- melt(normsettingsrightdf_OR,id.vars=c("Group_OR", "PID_OR"), measure.vars=c("RightDotV/VA", "RightDotVT/VTA" ), variable.name="condition", value.name="score")
...
normsettingsleft_formatted <- melt(normsettingsleftdf, id.vars=c("Group", "PID"), measure.vars=c("LeftDotV/VA", "LeftDotVT/VTA" ), variable.name="condition", value.name="score")
...
normsettingsleft_formatted_OR <- melt(normsettingsleftdf_OR, id.vars=c("Group_OR", "PID_OR"), measure.vars=c("LeftDotV/VA", "LeftDotVT/VTA" ), variable.name="condition", value.name="score")
...
normsettingsruler_formatted <- melt(normsettingsrulerdf, id.vars=c("Group", "PID"), measure.vars=c("RulerV/VA", "RulerVT/VTA" ), variable.name="condition", value.name="score")
...
normsettingsruler_formatted_OR <- melt(normsettingsrulerdf_OR, id.vars=c("Group_OR", "PID_OR"), measure.vars=c("RulerV/VA", "RulerVT/VTA" ), variable.name="condition",
value.name="score")
...
filtered.data_Baseline <- subset(subjective_data_factors,Condition=='Baseline Illusion ')
filtered.data_V_VA <- subset(subjective_data_factors,Condition=='V/VA Illusion ')
filtered.data_VT_VTA <- subset(subjective_data_factors,Condition=='VT/VTA Illusion ')

temp1 <- subset(subjective_data_factors,Group=='Audio' & Condition=='V/VA Illusion ')
temp2 <- subset(subjective_data_factors,Group=='No Audio' & Condition=='VT/VTA Illusion ')
filtered.data_VA_VT <- rbind(temp1,temp2)
filtered.data_VA_VT <- filtered.data_VA_VT[order(filtered.data_VA_VT$PID),]


#OR Sorting

filtered.dataOR_Baseline <- subset(subjective_data_factors_OR,Condition=='Baseline Illusion ')
filtered.dataOR_V_VA <- subset(subjective_data_factors_OR,Condition=='V/VA Illusion ')
filtered.dataOR_VT_VTA <- subset(subjective_data_factors_OR,Condition=='VT/VTA Illusion ')

temp1 <- subset(subjective_data_factors_OR,Group=='Audio' & Condition=='V/VA Illusion ')
temp2 <- subset(subjective_data_factors_OR,Group=='No Audio' & Condition=='VT/VTA Illusion ')
filtered.dataOR_VA_VT <- rbind(temp1,temp2)
filtered.dataOR_VA_VT <- filtered.dataOR_VA_VT[order(filtered.dataOR_VA_VT$PID),]
...
filtered.data_V_VA_BR <- subset(subjective_data_factors_BaselineRemoved,Condition=='V/VA Illusion ') 
filtered.data_VT_VTA_BR <- subset(subjective_data_factors_BaselineRemoved,Condition=='VT/VTA Illusion ') 

temp1 <- subset(subjective_data_factors_BaselineRemoved,Group=='Audio' & Condition=='V/VA Illusion ')
temp2 <- subset(subjective_data_factors_BaselineRemoved,Group=='No Audio' & Condition=='VT/VTA Illusion ')
filtered.data_VA_VT_BR <- rbind(temp1,temp2)
filtered.data_VA_VT_BR <- filtered.data_VA_VT_BR[order(filtered.data_VA_VT_BR$PID),]


#OR Sorting

filtered.dataOR_V_VA_BR <- subset(subjective_data_factors_OR_BaselineRemoved,Condition=='V/VA Illusion ') 
filtered.dataOR_VT_VTA_BR <- subset(subjective_data_factors_OR_BaselineRemoved,Condition=='VT/VTA Illusion ') 

temp1 <- subset(subjective_data_factors_OR_BaselineRemoved,Group=='Audio' & Condition=='V/VA Illusion ')
temp2 <- subset(subjective_data_factors_OR_BaselineRemoved,Group=='No Audio' & Condition=='VT/VTA Illusion ')
filtered.dataOR_VA_VT_BR <- rbind(temp1,temp2)
filtered.dataOR_VA_VT_BR <- filtered.dataOR_VA_VT_BR[order(filtered.data_VA_VT_BR$PID),]
...
layout(mat = layout.matrix,
       ...
       widths = c(3, 1)) # Widths of the columns
...
nbootstraps <- 10000
  ...
  par(mar = c(3, 0, 2.5, 0))
plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(0,1), ylim=c(0,1)) 
arrows(0.05,0.55,x1=0.05, y1=0.95,length=0.1,angle=30,lwd=3)
arrows(0.05,0.45,x1=0.05,y1=0.05,length=0.1,angle=30,lwd=3)
...
```{r figure2, fig.cap="Normalised Subjective Illusory Experience Score for V/VA and VT/VTA conditions. Group is indicated by colour, with red showing the no audio group and blue showing the audio group. Error bars show the standard error of the mean, which is shown by the circle and square respectively. Y-axis shows subjective illusion scale data after normalisation through subtraction of each participant’s baseline score from their V/VA and VT/VTA scores. Subjective illusion scale ranges from 0 indicating strongly disagreeing with the experience of finger stretching, 50 indicating a neutral opinion, and 100 indicating strongly agreeing with the experience of finger stretching. ", fig.align='center', echo=FALSE}
...
```{r figure3, fig.cap="Dot Touch Data in centimetres for V/VA and VT/VTA conditions for both left and right hand data, relative to baseline judgements. Group is indicated by colour, with red showing the no audio group and blue showing the audio group. Arrows denote the direction of finger length estimation (downward arrow showing overestimation, upward arrow showing underestimation).", fig.show='hold', fig.align='center', echo=FALSE}
...
```{r figure5, fig.cap="Ruler Judgement data in relative centimetres for V/VA and VT/VTA conditions. Group is indicated by colour, with red showing the no audio group and blue showing the audio group. Arrows denote direction of perceived finger length (downward arrow showing shorter perception, upward arrow showing longer perception).", fig.show='hold', fig.align='center', echo=FALSE}
